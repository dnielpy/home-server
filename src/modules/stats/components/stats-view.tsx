...
import { addNetworkHistoryPoint } from "@/src/modules/stats/data/network-history";
...
import { subscribeToStats } from "@/src/modules/stats/services/stats-service";
import { StorageMetricCard } from "@/src/modules/stats/components/storage-metric-card";
import type { SystemMetrics } from "@/src/modules/stats/types";
...
export const StatsView = ({ initialMetrics }: StatsViewProps) => {
  const [metrics, setMetrics] = useState(initialMetrics);
  const [error, setError] = useState<string | null>(null);
  const [history, setHistory] = useState(() => initialMetrics ? addNetworkHistoryPoint([], initialMetrics.network, new Date(initialMetrics.updatedAt)) : []);

  useEffect(() => {
    return subscribeToStats({
      onData: (payload) => {
        setMetrics(payload);
        setHistory((current) => addNetworkHistoryPoint(current, payload.network, new Date(payload.updatedAt)));
        setError(null);
      },
      onError: setError,
    });
  ...
  return (
    <div className="mx-auto max-w-[1440px]">
      ...
      <div className="mt-5"><NetworkChart history={history} network={metrics.network} /></div>
    </div>
  );
};
