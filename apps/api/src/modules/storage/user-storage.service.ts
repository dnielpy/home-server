...
const APPLICATION_DIRECTORIES = ["streamlt", "lgallery"] as const;
...
export class UserStorageService {
  ...
  public async ensureUserDirectory(userName: string) {
    ...
    await Promise.all(APPLICATION_DIRECTORIES.map((application) => mkdir(path.join(directory, application), { recursive: true, mode: 0o750 })));
  }
  ...
  public async createUserDirectory(userName: string) {
    ...
    try {
      ...
      if ((error as NodeJS.ErrnoException).code === "EEXIST") throw new ConflictException("Ya existe una carpeta para ese usuario.");
      throw error;
    }
    try {
      await Promise.all(APPLICATION_DIRECTORIES.map((application) => mkdir(path.join(this.userDirectory(userName), application), { mode: 0o750 })));
    } catch (error) {
  ...
  public async cleanupCreatedUserDirectory(userName: string) {
    ...
    await Promise.all(entries.filter((entry) => entry.startsWith("profile-")).map((entry) => unlink(path.join(directory, entry)).catch(() => undefined)));
    await Promise.all(APPLICATION_DIRECTORIES.map((application) => rmdir(path.join(directory, application)).catch(() => undefined)));
  ...
  public async savePhoto(userName: string, userId: string, upload: PhotoUpload) {
    ...
    if (upload.buffer.byteLength > MAX_PHOTO_BYTES) throw new PayloadTooLargeException("La foto no puede superar 5 MB.");
  ...
  private resolveRoot(configuredPath: string) {
    ...
    const isApiPackageDirectory = path.basename(workingDirectory) === "api" && path.basename(path.dirname(workingDirectory)) === "apps";
    return path.resolve(isApiPackageDirectory ? path.resolve(workingDirectory, "../..") : workingDirectory, configuredPath);
  }

  private photoPath(userName: string, filename: string) {
    const safeFilename = path.basename(filename);
    if (safeFilename !== filename || !filename.startsWith("profile-")) throw new NotFoundException("Foto no encontrada.");
