...
function ProfileAvatar({ profile, selected = false }: { profile: LoginProfile; selected?: boolean }) {
  ...
  const color = [
    "from-[#d84c59] to-[#8f2031]",
    "from-[#538cc6] to-[#1d4775]",
    "from-[#9773c8] to-[#4d3474]",
    "from-[#d39a50] to-[#77451d}",
  ][profile.name.codePointAt(0)! % 4];
  ...
  return profile.photoUrl ? (
    ...
    <span
      className={`grid aspect-square w-full place-items-center bg-gradient-to-br ${color} text-4xl font-medium text-white transition duration-200 ${selected ? "ring-2 ring-white ring-offset-4 ring-offset-[#141414]" : ""}`}
      aria-hidden="true"
    >
      {initials}
    </span>
...
export function LoginForm({ nextPath, profiles }: LoginFormProps) {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#141414] text-white">
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_top,rgba(229,9,20,0.13),transparent_42%)]"
      />
      <header className="relative flex h-16 items-center px-5 sm:h-20 sm:px-10 lg:px-[4vw]">
        <span className="flex items-center gap-2.5 text-[#e50914]">
          ...
          <span className="text-xl font-black tracking-[-0.08em] uppercase sm:text-2xl">Home Server</span>
        </span>
      </header>
      ...
      <div className="relative mx-auto flex min-h-[calc(100vh-4rem)] max-w-5xl items-center justify-center px-5 pb-20 sm:min-h-[calc(100vh-5rem)] sm:px-10">
        {!selectedProfile ? (
          <section className="w-full text-center" aria-labelledby="profiles-title">
            <h1 id="profiles-title" className="text-3xl font-medium tracking-tight sm:text-5xl">
              ¿Quién está usando Home Server?
            </h1>
            {profiles.length > 0 ? (
              <div className="mx-auto mt-9 grid max-w-3xl grid-cols-2 justify-center gap-x-4 gap-y-7 sm:mt-12 sm:grid-cols-3 sm:gap-x-7 md:grid-cols-4">
                {profiles.map((profile) => (
                  <button
                    ...
                    className="group min-w-0 text-center outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-4 focus-visible:ring-offset-[#141414]"
                  >
                    ...
                    <span className="mt-3 block truncate text-sm text-[#808080] transition-colors group-hover:text-white sm:mt-4 sm:text-base">
                      {profile.name}
                    </span>
                  </button>
                ))}
              ...
              <div className="mx-auto mt-9 max-w-sm text-left">
                ...
                <form className="mt-5 space-y-3" onSubmit={(event) => void submit(event)}>
                  <input
                    ...
                    placeholder="Usuario"
                    className="h-12 w-full border border-[#555] bg-[#333] px-4 text-white outline-none placeholder:text-[#8c8c8c] focus:border-white"
                  />
                  <input
                    required
                    type="password"
                  ...
                  />
                  {error && (
                    <p role="alert" className="text-sm text-[#e87c03]">
                      {error}
                    </p>
                  )}
                  <button
                    disabled={pending}
                    type="submit"
                    className="h-11 w-full bg-[#e50914] text-sm font-bold transition hover:bg-[#f6121d] disabled:cursor-not-allowed disabled:opacity-60"
                  >
          ...
          <section className="w-full max-w-sm text-center" aria-labelledby="password-title">
            <button
              ...
              className="mb-8 inline-flex items-center gap-2 text-sm text-[#b3b3b3] transition hover:text-white focus-visible:ring-2 focus-visible:ring-white focus-visible:outline-none"
            >
            ...
            <h1 id="password-title" className="mt-6 text-3xl font-medium tracking-tight">
            ...
            <form className="mx-auto mt-7 max-w-[18rem] space-y-3" onSubmit={(event) => void submit(event)}>
              <label className="sr-only" htmlFor="profile-password">
                Contraseña
              </label>
              <input
                id="profile-password"
              ...
              />
              {error && (
                <p role="alert" className="text-left text-sm text-[#e87c03]">
                  {error}
                </p>
              )}
              <button
                disabled={pending}
                type="submit"
                className="h-9 w-full rounded-md bg-[#e50914] text-sm font-bold transition hover:bg-[#f6121d] focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-[#141414] focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-60"
              >
                {pending ? "Entrando…" : "Entrar"}
              </button>
            </form>
