...
function mapUserUrls<T extends { users?: Array<{ id: string; photoUrl: string | null }>; user?: { id: string; photoUrl: string | null } }>(value: T) {
  const map = (user: { id: string; photoUrl: string | null }) => ({ ...user, photoUrl: user.photoUrl ? `/api/users/${user.id}/photo` : null });
  return { ...value, ...(value.users ? { users: value.users.map(map) } : {}), ...(value.user ? { user: map(value.user) } : {}) };
}
...
export async function POST(request: Request) {
  ...
  const response = await apiFetch("/v1/users", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
